  ...
  return <div className="min-h-screen pb-12"><AppHeader isAdmin={me.roles.includes("admin")} accountId={me.profile?.account_id} />
    <main className="mx-auto max-w-4xl space-y-5 px-4 py-6 md:px-6 md:py-10">
      ...
      {!application || application.status === "declined"
        ? <ApplicationForm userId={me.profile.id} previous={application} eligibility={data?.eligibility} onDone={refresh} />
...
function ApplicationForm({ userId, previous, eligibility, onDone }: { userId: string; previous: any; eligibility: any; onDone: () => void }) {
  ...
  return <Card className="rounded-2xl p-5 md:p-6">
    ...
    <div className={`mt-3 rounded-xl border p-3 text-sm ${eligibility?.eligible ? "border-emerald-500/30 bg-emerald-500/10" : "border-amber-500/30 bg-amber-500/10"}`}>
      <div className="flex items-start gap-2"><AlertCircle className="mt-0.5 h-4 w-4 shrink-0" /><div>
        <div className="font-medium">Insurance approval requirements</div>
        <p className="mt-1 text-muted-foreground">Your account must be more than 30 days old, with at least R1,000 in completed deposits during the last 30 days.</p>
        {eligibility && <p className="mt-1 text-xs">Current: {eligibility.accountAgeDays} account days · {formatMoney(Number(eligibility.depositsLast30Days), "ZAR")} deposited.</p>}
      </div></div>
    </div>
...
function ApprovedDashboard({ application, claims, userId, onDone }: { application: any; claims: any[]; userId: string; onDone: () => void }) {
  ...
  const [busy, setBusy] = useState(false);
  const activeRepayment = claims.find(claim => claim.repayment_status === "active");
  ...
  return <div className="space-y-5">
    <Card className="overflow-hidden rounded-2xl"><div className="gradient-brand p-6 text-white"><div className="text-sm text-white/80">Insurance credit facility</div><div className="mt-1 font-display text-4xl font-bold">{formatMoney(Number(application.credit_limit), "ZAR")}</div><div className="mt-2 text-sm">Available to claim: {formatMoney(Number(application.credit_available), "ZAR")}</div></div><div className="p-5"><div className="flex items-center gap-2 text-sm"><CheckCircle2 className="h-5 w-5 text-emerald-600" />Cover approved for {application.selected_items.length} appliance{application.selected_items.length === 1 ? "" : "s"}.</div>
      {activeRepayment && <div className="mt-4 rounded-xl border border-amber-500/30 bg-amber-500/10 p-3 text-sm"><div className="font-semibold">Insurance credit repayment</div><p className="mt-1 text-muted-foreground">A 40% credit charge is split across three repayments. Each repayment includes the principal plus 13.33% of the claim. Repayments are automatically taken when withdrawable funds become available.</p><div className="mt-2 flex justify-between gap-3"><span>Paid {formatMoney(Number(activeRepayment.repayment_paid), "ZAR")}</span><strong>Outstanding {formatMoney(Math.max(0, Number(activeRepayment.repayment_total)-Number(activeRepayment.repayment_paid)), "ZAR")}</strong></div></div>}
      <Button className="mt-4 gradient-brand text-white" disabled={Number(application.credit_available) <= 0 || !!activeRepayment} onClick={() => setShowClaim(value => !value)}><ShieldCheck className="mr-2 h-4 w-4" />{activeRepayment ? "Repayment in progress" : "Claim insurance"}</Button></div></Card>
