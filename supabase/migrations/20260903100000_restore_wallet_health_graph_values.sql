-- Restore the full ZAR wallet value written to the wallet-health history.
-- The maturity/presence migration kept the withdrawable calculation correct,
-- but accidentally omitted wallet_value_zar from the snapshot upsert. That
-- left newly-created chart points at the column default of zero.
...
BEGIN
  ...
  v_withdrawable := greatest(0, v_wallet_value - v_locked);
  ...
  INSERT INTO public.wallet_health_daily (
    user_id, snapshot_date, wallet_value_zar, withdrawable_zar,
    wallet_health, daily_top_ups, withdrawals, penalties
  ) VALUES (
    p_user_id, v_snapshot_date, v_wallet_value, v_withdrawable,
    v_health, v_top_ups, v_withdrawals, v_penalties
  )
...
REVOKE ALL ON FUNCTION public.record_wallet_health_snapshot(uuid)
  FROM PUBLIC, anon, authenticated;

-- Repair today's point immediately. Historical rows are deliberately left
-- unchanged because reconstructing an exact past balance from today's state
-- could create misleading account history.
DO $$
DECLARE
  v_profile record;
BEGIN
  FOR v_profile IN SELECT id FROM public.profiles LOOP
    PERFORM public.record_wallet_health_snapshot(v_profile.id);
  END LOOP;
END;
$$;
